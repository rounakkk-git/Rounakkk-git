# 👋 Hi, I'm Rounak Narain

💻 Full Stack Developer focused on building clean, responsive, and user-friendly web applications.  
🎨 I enjoy turning UI ideas into functional products and understanding applications end-to-end.

📍 Dhanbad, Jharkhand, India  
📧 rounakinvertis@gmail.com  
🔗 https://linkedin.com/in/rounak-narain-73848224b  

---

## 🚀 About Me

✨ Full Stack Trainee with hands-on development experience  
🎯 Strong focus on UI, usability, and clean layouts  
⚡ Comfortable in fast-paced environments  
🔐 Growing interest in application security and best practices  

---

## 🛠 Tech Stack

### 🎨 Frontend
🟢 HTML5  
🟢 CSS3  
🟢 JavaScript (ES6+)  
🟢 React.js  

### 🧩 UI & Development
📱 Responsive Design  
🧱 Component-based Architecture  
🧠 Client-side Rendering  

### 🧰 Tools
🔧 Git & GitHub  
🖥 VS Code  
🔍 Browser DevTools  

### 🔐 Other Knowledge
⚙ Application Workflow  
🛡 Cyber Security Fundamentals  
⚠ Security Awareness & Threat Concepts  

---

## 💼 Experience

### 🏢 Stamens Software  
**Full Stack Trainee** | April 2025 – Present  

🧩 Developed responsive UI components using HTML, CSS, JavaScript, and React  
🎨 Designed interfaces focused on usability and clean structure  
🚀 Learned real-world application flow and feature integration  
🌐 Gained understanding of production-level applications  

---

## 🎓 Education

🎓 **B.Sc. in Forensic Science**  
🏫 Invertis University  
📅 Graduated: May 2024  

---

## 📜 Certification

📘 **Cyber Security Program (6 Months)**  
🔹 Cyber security fundamentals  
🔹 Common vulnerabilities & secure practices  

---

## 💡 Strengths

🎯 Strong eye for UI and design logic  
⚡ Quick decision-making under pressure  
🧠 Analytical problem-solving mindset  
📈 Adaptable and eager to learn new technologies  

---

## 📈 Currently Working On

🚀 Improving React & JavaScript logic  
🧪 Building real-world frontend projects  
🔄 Understanding full-stack application flow  
🔐 Learning secure development basics  

## ☁ Deployment & Cloud Exposure

🚀 Familiar with deploying frontend and backend applications to cloud platforms  

🔷 **Microsoft Azure**  
📦 App deployment basics  
🌐 Hosting frontend & backend services  
⚙ Understanding cloud-based application flow  

---

## 🧰 Development Tools

🧠 Tools I regularly use to build, test, debug, and collaborate effectively  

🔧 **Git** – Version control & clean commit practices  
🐙 **GitHub** – Repository management & collaboration  
📬 **Postman** – API testing and request validation  
🖥 **VS Code** – Daily development environment  
🔍 **Browser DevTools** – Debugging UI & performance  

---

## 🤝 How I Work

🤝 Collaborate closely with team members and stakeholders  
🧠 Understand requirements before implementing solutions  
🛠 Improve existing codebases instead of rewriting blindly  
📦 Write code with long-term maintainability in mind  
🎯 Focus on usability, clarity, and clean UI logic  


---

⭐ Check out my repositories and feel free to connect.
